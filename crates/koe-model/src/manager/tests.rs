...
use crate::{
    AsrSessionSettings, DigestAllowlist, FixtureFoundryAdapter, KoeModelManager, ModelError,
    ModelManager, ModelScope, ModelSelector, fixture::CountingAdapter, fixture_transcribe,
...
async fn online_install_then_offline_transcription_succeeds() {
    ...
    manager.load(&installed.id).await.expect("offline load");
