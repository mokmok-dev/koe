#!/usr/bin/env bash
# Linux HIL gate for the exact AppImage and signed CLI release artifacts.
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
bin="${KOE_HIL_BIN:?KOE_HIL_BIN must name the downloaded release CLI}"
appimage="${KOE_HIL_PACKAGE:?KOE_HIL_PACKAGE must name the downloaded AppImage}"
...
(( duration_secs >= 3600 )) || { echo "hil-linux: release soak must run for at least 3600 seconds" >&2; exit 2; }
mkdir -p "$root" "$repo_root/target/hil"

for file in "$bin" "$appimage" "$metadata" "$expired_metadata" "$tamper_metadata" "$system_wav" "$mic_wav"; do
    [[ -f "$file" ]] || { echo "hil-linux: missing required artifact/fixture: $file" >&2; exit 2; }
done
[[ -x "$mic_player" ]] || { echo "hil-linux: KOE_HIL_MIC_PLAYER is required" >&2; exit 2; }
[[ -x "$offline_runner" && -n "$model_selector" ]] || { echo "hil-linux: offline runner and model selector are required" >&2; exit 2; }
[[ -x "$platform_gates" ]] || { echo "hil-linux: KOE_HIL_PLATFORM_GATES is required" >&2; exit 2; }
command -v pw-play >/dev/null 2>&1 || { echo "hil-linux: pw-play is required" >&2; exit 2; }
chmod +x "$bin" "$appimage"

# Install/smoke/uninstall the exact AppImage in an isolated directory.
install_dir="$(mktemp -d)"
installed="$install_dir/koe.AppImage"
cp "$appimage" "$installed"
chmod 500 "$installed"
set +e
APPIMAGE_EXTRACT_AND_RUN=1 timeout 10s "$installed" >/dev/null 2>&1
launch_status=$?
set -e
if [[ $launch_status -ne 0 && $launch_status -ne 124 ]]; then
    echo "hil-linux: AppImage failed to launch ($launch_status)" >&2
    exit 1
fi
"$platform_gates" "$bin" "$installed" "$root"
rm -rf "$install_dir"

# The pinned key verifies the package inventory entry and designated CLI.
"$bin" update --data-root "$root" verify --metadata "$metadata" \
    --target "$appimage" --target-name "$(basename "$appimage")"
...
expect_update_rejection() {
    ...
    if "$@" >"$root/rejection.out" 2>"$root/rejection.err"; then
        echo "hil-linux: update rejection $expected unexpectedly succeeded" >&2
        exit 1
    fi
...
[[ -n "$mic_id" && -n "$system_id" ]] || { echo "hil-linux: mic and system devices required" >&2; exit 1; }
...
[[ -n "$ready" ]] || { echo "hil-linux: capture did not become ready" >&2; kill "$record_pid"; exit 1; }
"$mic_player" "$mic_wav" &
mic_pid=$!
pw-play "$system_wav" &
system_pid=$!
